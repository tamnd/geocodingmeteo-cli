...
describes what users get. Pick a format with `-o`, or let geocodingmeteo choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
geocodingmeteo <command> -o table   # aligned columns for reading
geocodingmeteo <command> -o jsonl   # one JSON object per line, for piping
geocodingmeteo <command> -o json    # a single JSON array
geocodingmeteo <command> -o csv     # spreadsheet friendly
geocodingmeteo <command> -o tsv     # tab-separated
geocodingmeteo <command> -o url     # just the URL column
geocodingmeteo <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
geocodingmeteo <command> --fields id,title,url
```
...
```bash
geocodingmeteo <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
geocodingmeteo <command>            # a table, because this is a terminal
geocodingmeteo <command> | wc -l    # JSONL, because this is a pipe
```
